...
// The form includes fields for name, email, and organization.'use client';
import { useState } from 'react';
import axios from 'axios';

export default function PublisherForm() {
  const [form, setForm] = useState({ username: '', email: '', organization: '' });
  const [message, setMessage] = useState('');
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-4 shadow rounded space-y-3">
      ...
      <input name="organization" placeholder="Organization" onChange={handleChange} className="w-full p-2 border rounded" />
